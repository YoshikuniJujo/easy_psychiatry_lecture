...
subtitle :: String
subtitle = "2017年 5月17日 第10回 統合失調症"
...
case1 = [ \t -> do
	writeTopTitle t "事例1"
	text t "", \t -> do
	text t "太郎さん(26歳)は、役者を目指して、大学をやめました", \t -> do
	text t "いくつかのオーディションを受けましたが、どれも落ち", \t -> do
	text t "最後のチャンスのコンテストで、調子をくずしました", \t -> do
	text t "家に帰ったあとは、うずくまったまま", \t -> do
	text t "「大変なことになった」とつぶやいたり", \t -> do
	text t "突然「だまれ」とさけんだりしています", \t -> do
	text t "しばらくすると、じっとして動かなくなり", \t -> do
	itext t 1 "食事をすることも、水を飲むこともありません", \t -> do
	text t "その日の深夜には、「いっそ殺してくれ」と、さわぎだし", \t -> do
	itext t 1 "なだめることもできなくなりました"
	]
...
case2 = [ \t -> do
	writeTopTitle t "事例2"
	text t "", \t -> do
	text t "花子さん(28歳)は中規模の会社の事務員をしていました", \t -> do
	text t "人間関係のトラブルで仕事をやめ、家の手伝いをしていました", \t -> do
	text t "ある日の飲み会で、隣りにすわった男性が好きになりました", \t -> do
	text t "家族には、その男性からプロポーズを受けた、と話しました", \t -> do
	text t "しばらくして、その男性の勤務先から電話がかかってきました", \t -> do
	text t "花子さんが、その男性に会わせてくれるようにと、", \t -> do
	itext t 1 "受付けから動かなくなった、とのことでした", \t -> do
	text t "花子さんは、その男性とテレパシーで話し合ったとのこと", \t -> do
	text t "家族がいくら説得しても、かたくなに、言いはりました"
	]
